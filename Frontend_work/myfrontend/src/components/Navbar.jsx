    ...
    <>
      <nav className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 lg:px-10 h-20 flex items-center shadow-sm">
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center">
          ...
          {/* Search Overlay - Styled with Cyan focus */}
          {isSearchOpen && (
          ...
          {/* Logo - Matches Home.jsx Cyan hover */}
          <Link to="/" className="text-2xl font-black tracking-tighter text-slate-950 uppercase hover:text-cyan-600 transition-colors duration-300">
            Devixa
          </Link>
          
          {/* Desktop Menu - Cyan indicator lines */}
          <div className="hidden lg:flex gap-10 items-center text-[10px] font-black uppercase tracking-[0.25em] text-slate-500">
            {navItems.map((item, idx) => (
              ...
              >
                <button className={`relative hover:text-cyan-600 transition-all duration-300 cursor-pointer outline-none h-full flex items-center ${activeDropdown === item.label ? 'text-cyan-600' : ''}`}>
                  {item.label}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative group flex items-center h-20">
              ...
              <div className="absolute top-20 right-0 w-48 hidden group-hover:block z-50 animate-in fade-in slide-in-from-top-1 duration-200">
                <div className="bg-white border border-slate-100 shadow-2xl rounded-sm overflow-hidden">
                  {!token ? (
                    <>
                      <Link to="/login" className="block px-6 py-4 text-[9px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 hover:text-cyan-600 transition-colors">Login</Link>
                      <Link to="/signup" className="block px-6 py-4 text-[9px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 hover:text-cyan-600 border-t border-slate-50 transition-colors">Sign Up</Link>
                    </>
                  ) : (
                    <>
                      <Link to="/profile" className="block px-6 py-4 text-[9px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 hover:text-cyan-600 transition-colors">My Profile</Link>
                      <button 
            ...
            {/* Main CTA - Matches Home.jsx Hero Button */}
      ...
      {/* Mega Menu Panel - Cyan accent styling */}
      {activeDropdown && (
        <div 
